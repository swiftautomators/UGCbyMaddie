...
const StrategyAI: React.FC = React.memo(() => {
  ...
  return (
    <section id="ai" className="py-24 px-6 relative overflow-hidden" aria-labelledby="ai-heading">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#6b705c]/10 rounded-full blur-[120px] -z-10" />
      <div className="max-w-4xl mx-auto">
        <div className="glass-card p-8 md:p-12 rounded-[2rem] border-[#6b705c]/10">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 text-[#cb997e] font-bold text-xs uppercase mb-4">
              ...
              <div className="relative group">
                <input
                  ...
                  className="w-full bg-black/50 border border-white/10 rounded-2xl px-6 py-4 focus:border-[#cb997e] focus:outline-none transition-colors group-hover:border-white/20"
                />
                <button
                  onClick={handleGenerate}
                  disabled={isLoading}
                  aria-label="Generate AI strategy"
                  className="absolute right-2 top-2 h-[calc(100%-1rem)] px-6 bg-[#6b705c] rounded-xl font-bold flex items-center gap-2 hover:bg-[#a5a58d] disabled:opacity-50 transition-colors"
                >
            ...
            <div
              className="w-full md:w-80 min-h-[300px] glass-card rounded-2xl p-6 bg-black/40 border-[#6b705c]/10 relative flex flex-col items-center justify-center overflow-hidden"
              aria-live="polite"
            >
              <AnimatePresence mode="wait">
                {!strategy && !isLoading && !error && (
                  ...
                  >
                    <div className="w-16 h-16 rounded-full bg-[#6b705c]/10 flex items-center justify-center mx-auto mb-4">
                      <Send className="text-[#cb997e]/50" aria-hidden="true" />
                    </div>
                ...
                {isLoading && (
                  ...
                  >
                    <Loader2 className="animate-spin text-[#cb997e] mb-4" size={32} aria-hidden="true" />
                    <p className="text-xs text-[#cb997e] font-bold uppercase tracking-widest animate-pulse">Analyzing Funnel Stage...</p>
                  </motion.div>
                )}
                ...
                {strategy && !isLoading && !error && (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-[10px] font-black text-[#cb997e] uppercase mb-1">Winning Hook</div>
                      <div className="text-sm font-medium text-white italic">&ldquo;{strategy.hook}&rdquo;</div>
                    </div>
                    <div>
                      <div className="text-[10px] font-black text-[#cb997e] uppercase mb-1">Direct Angle</div>
                      <div className="text-sm font-medium text-white">{strategy.angle}</div>
                    </div>
                    <div className="pt-4 border-t border-white/5">
                      <div className="text-[10px] font-black text-[#a5a58d] uppercase mb-1">Conversion Logic</div>
