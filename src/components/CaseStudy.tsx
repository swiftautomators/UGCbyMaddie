...
const FeatureItem = React.memo(({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <div className="flex gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-purple-500/30 transition-colors group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 id="results-heading" className="text-4xl md:text-6xl font-heading font-extrabold mb-8 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">Through UGC Strategy</span>
            </h2>
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-3 text-lg font-bold text-white hover:text-purple-400 transition-colors group"
              ...
              <div className="h-48 flex items-end gap-3 mb-8" role="img" aria-label="Bar chart showing growth over time">
                {[40, 65, 45, 90, 70, 100, 85].map((h, i) => (
                  <motion.div
                    ...
                    className={`flex-1 rounded-t-lg ${i === 5 ? 'bg-gradient-to-t from-purple-600 to-indigo-400' : 'bg-white/10'}`}
                  />
                ))}
              </div>

              <blockquote className="p-6 rounded-2xl bg-white/5 border-l-4 border-purple-500 italic text-gray-300">
